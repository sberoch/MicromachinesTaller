...
void Track::_setFixtureDef(std::shared_ptr<Configuration> configuration){
    ...
    std::shared_ptr<GroundAreaFUD> gaFUD(new GroundAreaFUD(0.9f, false, _id));
    _fixture->SetUserData(gaFUD.get());
}

Track::Track(std::shared_ptr<b2World> world, size_t id, int type, float x_init, float y_init, float angle_init, std::shared_ptr<Configuration> configuration) :

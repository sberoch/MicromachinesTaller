...
std::shared_ptr<Car> World::createCar(size_t id, json j){
    ...
    std::shared_ptr<Car> newCar(new Car(_world, id, x_init, y_init, angle_init * DEGTORAD,
                                           _track.size(), _configuration));
    _cars.push_back(newCar);
    return newCar;
}

void World::createTrack(){
    std::ifstream i(MISC_DIR + std::string(_configuration->getMapToUse()));
    json j; i >> j;
